...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS file_likes (
        ...
        UNIQUE KEY unique_like (user_id, file_hash),
        INDEX (file_hash)
    )");

    $pdo->exec("CREATE TABLE IF NOT EXISTS file_comments (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id VARCHAR(50) NOT NULL,
        file_hash VARCHAR(255) NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'toggle_like') {
    ...
    $check = $pdo->prepare("SELECT id FROM file_likes WHERE user_id=? AND file_hash=?");
    $check->execute([$user_id, $file_hash]);
    $existing = $check->fetch();

    if ($existing) {
        $pdo->prepare("DELETE FROM file_likes WHERE user_id=? AND file_hash=?")->execute([$user_id, $file_hash]);
        $liked = false;
    } else {
        $pdo->prepare("INSERT IGNORE INTO file_likes (user_id, file_hash) VALUES (?,?)")->execute([$user_id, $file_hash]);
        $liked = true;
    }

    $cntStmt = $pdo->prepare("SELECT COUNT(*) FROM file_likes WHERE file_hash=?");
    $cntStmt->execute([$file_hash]);
...
if ($action === 'get_likes') {
    header('Content-Type: application/json');
    $file_hash = trim($_GET['file_hash'] ?? '');
    if (!$file_hash) { echo json_encode(['ok'=>false]); exit; }

    $cntStmt = $pdo->prepare("SELECT COUNT(*) FROM file_likes WHERE file_hash=?");
    $cntStmt->execute([$file_hash]);
    $count = (int)$cntStmt->fetchColumn();

    $likedStmt = $pdo->prepare("SELECT id FROM file_likes WHERE user_id=? AND file_hash=? LIMIT 1");
    $likedStmt->execute([$user_id, $file_hash]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add_comment') {
    ...
    if (!$file_hash || !$comment) { echo json_encode(['ok'=>false,'error'=>'Missing fields']); exit; }
    if (mb_strlen($comment) > 500) { echo json_encode(['ok'=>false,'error'=>'Comment too long (max 500 chars)']); exit; }

    $ins = $pdo->prepare("INSERT INTO file_comments (user_id, file_hash, comment) VALUES (?,?,?)");
    $ins->execute([$user_id, $file_hash, $comment]);
...
if ($action === 'get_comments') {
    ...
    $stmt = $pdo->prepare(
        "SELECT id, user_id, comment, created_at FROM file_comments
          WHERE file_hash=? ORDER BY created_at DESC LIMIT $limit OFFSET $offset"
    );
    ...
    foreach ($rows as &$row) {
        ...
        $row['user_label'] = ($row['user_id'] === $user_id) ? 'You' : 'usr_' . substr($row['user_id'], -4);
    }

    $cntStmt = $pdo->prepare("SELECT COUNT(*) FROM file_comments WHERE file_hash=?");
    $cntStmt->execute([$file_hash]);
    $total = (int)$cntStmt->fetchColumn();

    echo json_encode(['ok'=>true,'comments'=>$rows,'total'=>$total,'page'=>$page]);
    exit;
}

// ── POST ?action=record_preference ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'record_preference') {    header('Content-Type: application/json');
    $input     = json_decode(file_get_contents('php://input'), true) ?? [];
    $file_hash = trim($input['file_hash'] ?? '');
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── LIKES & COMMENTS ── */
  .action-btn.liked .action-icon { background: rgba(255,68,102,0.25); border-color: rgba(255,68,102,0.5); }
  .action-btn.liked .action-icon svg { color: #ff4466; }
  .action-btn.liked .action-label { color: #ff6680; }
  ...
  .comment-panel.open { pointer-events: all; }

  .comment-backdrop {
    position: absolute; inset: 0;
    background: rgba(0,0,0,0); transition: background 0.35s;
  }
  .comment-panel.open .comment-backdrop { background: rgba(0,0,0,0.6); }
  ...
  .comment-panel.open .comment-sheet { transform: translateY(0); }

  .comment-handle {
    width: 36px; height: 4px; border-radius: 2px;
    background: rgba(255,255,255,0.15); margin: 12px auto 0; flex-shrink: 0;
  }
  .comment-header {
  ...
  .comment-item.mine { background: rgba(0,245,160,0.05); border-color: rgba(0,245,160,0.15); }
  ...
  .comment-item.mine .comment-author { color: var(--accent); }
  .comment-text { font-size: 14px; line-height: 1.5; color: var(--text); word-break: break-word; }

  .comment-empty {
    text-align: center; padding: 32px 16px;
    font-family: 'Space Mono', monospace; font-size: 11px; color: var(--muted); letter-spacing: 1px;
  }
  ...
  .comment-textarea:focus { border-color: rgba(0,245,160,0.4); }
  ...
  .comment-send:not(:disabled):active { transform: scale(0.9); }

  .comment-load-more {
    background: none; border: 1px solid var(--border); border-radius: 8px;
    color: var(--muted); font-family: 'Space Mono', monospace; font-size: 10px;
    padding: 8px; cursor: pointer; width: 100%; letter-spacing: 1px;
    transition: border-color 0.2s, color 0.2s;
  }
  .comment-load-more:hover { border-color: var(--accent); color: var(--accent); }
...
<div id="comment-panel" class="comment-panel">
  <div class="comment-backdrop" id="comment-backdrop"></div>
  <div class="comment-sheet">
    <div class="comment-handle"></div>
    <div class="comment-header">
      <span class="comment-header-title">COMMENTS — <span id="comment-count-header">0</span></span>
      <button class="comment-close" id="comment-close-btn" aria-label="Close">&times;</button>
    </div>
    <div class="comment-list" id="comment-list"></div>
    <div class="comment-input-row">
      <textarea class="comment-textarea" id="comment-input" placeholder="Add a comment…" rows="1" maxlength="500"></textarea>
      <button class="comment-send" id="comment-send-btn" disabled aria-label="Send">
...
async function recordPreference(fileHash, cardEl) {
  if (!fileHash) return;
  try {
    const res  = await fetch('feed_full.php?action=record_preference', {
      method: 'POST',
...
function makeCard(entry, index, isRandom = false) {
  ...
  // ── LIKE button ──
  const likeBtn = makeActionBtn('Like', `
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
    </svg>`);
  likeBtn.onclick = (e) => {
    e.preventDefault(); e.stopPropagation();
    toggleLike(fileHash, likeBtn);
  };
  actions.appendChild(likeBtn);
  // Fetch initial state
  if (fileHash) fetchLikeState(fileHash, likeBtn);

  // ── COMMENT button ──
  const cmtIconSvg = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
    </svg>`;
  const cmtBtn = makeActionBtn('', cmtIconSvg);
  const cmtLbl = cmtBtn.querySelector('.action-label');
  cmtLbl.className = 'action-label comment-count-lbl';
  cmtLbl.textContent = '';
  cmtBtn.onclick = (e) => {
    e.preventDefault(); e.stopPropagation();
    openComments(fileHash);
  };
  actions.appendChild(cmtBtn);
  if (fileHash) fetchCommentCount(fileHash, card);
...
async function loadMoreEntries() {
  ...
  try {
    ...
    const [pageData, randomEntry] = await Promise.all([
      fetch(`feed_full.php?action=feed&page=${currentPage + 1}${seen}`).then(r => r.json()),
      fetch(`feed_full.php?action=random${seen}`).then(r => r.json())
    ]);
    currentPage++;
...
async function init() {
  ...
  try {
    ...
    const data = await fetch('feed_full.php?action=feed&page=1&reset=1').then(r => r.json());
...
const commentClosBtn  = document.getElementById('comment-close-btn');
const commentList     = document.getElementById('comment-list');
const commentInput    = document.getElementById('comment-input');
const commentSendBtn  = document.getElementById('comment-send-btn');
const commentCountHdr = document.getElementById('comment-count-header');

let activeCommentHash = null;
let commentPage       = 1;
let commentTotal      = 0;
let commentLoading    = false;

// ── Open / close panel ──────────────────────────────
function openComments(fileHash) {
  activeCommentHash = fileHash;
  commentPage       = 1;
  commentTotal      = 0;
  commentList.innerHTML = '';
  commentInput.value    = '';
  commentSendBtn.disabled = true;
  commentCountHdr.textContent = '…';
  commentPanel.classList.add('open');
  document.body.style.overflow = 'hidden';
  loadComments(fileHash, 1, true);
}

function closeComments() {
  commentPanel.classList.remove('open');
  document.body.style.overflow = '';
  activeCommentHash = null;
}

commentBackdrop.addEventListener('click', closeComments);
commentClosBtn.addEventListener('click', closeComments);

// ── Input auto-grow & send enable ───────────────────
commentInput.addEventListener('input', function() {
  this.style.height = 'auto';
  this.style.height = Math.min(this.scrollHeight, 120) + 'px';
  commentSendBtn.disabled = !this.value.trim();
});
...
// ── Load comments ────────────────────────────────────
async function loadComments(fileHash, page, reset = false) {
  if (commentLoading) return;
  commentLoading = true;
  try {
    const r    = await fetch(`feed_full.php?action=get_comments&file_hash=${encodeURIComponent(fileHash)}&page=${page}`);
    const data = await r.json();
    if (!data.ok) return;

    commentTotal = data.total;
    commentCountHdr.textContent = data.total;
    commentPage = page;

    if (reset) {
      // Remove old load-more button if any
      const old = commentList.querySelector('.comment-load-more');
      if (old) old.remove();
    }
    ...
    for (const c of data.comments) {
      fragment.appendChild(buildCommentEl(c));
    }

    if (reset) {
      // Prepend (comments are newest-first from server)
      commentList.prepend(fragment);
    } else {
      // Prepending older comments above existing
      commentList.prepend(fragment);
    }

    // Load-more button (there are more pages)
    if ((data.comments.length === 20) && (commentTotal > page * 20)) {
      const btn = document.createElement('button');
      btn.className = 'comment-load-more';
      btn.textContent = 'LOAD MORE';
      btn.onclick = () => {
        btn.remove();
        loadComments(fileHash, page + 1, false);
      };
      commentList.prepend(btn);
    }
...
function buildCommentEl(c) {
  ...
  item.className = 'comment-item' + (c.is_mine ? ' mine' : '');
  ...
  author.textContent = c.user_label || (c.is_mine ? 'You' : 'User');

  const ts   = document.createElement('span');
  ts.textContent = formatTimestamp(c.created_at);
...
async function sendComment() {
  ...
  if (!text || !activeCommentHash || commentSendBtn.disabled) return;
  commentSendBtn.disabled = true;

  try {
    const r    = await fetch('feed_full.php?action=add_comment', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({file_hash: activeCommentHash, comment: text})
    });
    const data = await r.json();
    ...
    commentTotal++;
    commentCountHdr.textContent = commentTotal;

    // Append new comment at bottom (newest is at top from server, but user's own goes at end for UX)
    ...
    // Update like/comment counter badge on active card
    updateCommentBadge(activeCommentHash, commentTotal);

  } catch(e) { showToast('Failed to send comment'); }
  finally { commentSendBtn.disabled = false; }
}

function updateCommentBadge(fileHash, count) {
  const card = feed.querySelector(`.card[data-hash="${fileHash}"]`);
  if (!card) return;
  const lbl = card.querySelector('.comment-count-lbl');
  if (lbl) lbl.textContent = count || '';
}

// ── Like toggle ───────────────────────────────────────
async function toggleLike(fileHash, btn) {
  btn.disabled = true;
  try {
    const r    = await fetch('feed_full.php?action=toggle_like', {
...
function applyLikeState(btn, liked, count) {
  ...
  if (lbl)  lbl.textContent = count > 0 ? count : 'Like';
}

async function fetchLikeState(fileHash, btn) {
  try {
    const r    = await fetch(`feed_full.php?action=get_likes&file_hash=${encodeURIComponent(fileHash)}`);
    const data = await r.json();
    if (data.ok) applyLikeState(btn, data.liked, data.count);
  } catch(e) {}
}

async function fetchCommentCount(fileHash, card) {
  try {
    const r    = await fetch(`feed_full.php?action=get_comments&file_hash=${encodeURIComponent(fileHash)}&page=1`);
    const data = await r.json();
    if (data.ok) updateCommentBadge(fileHash, data.total);
  } catch(e) {}
}
